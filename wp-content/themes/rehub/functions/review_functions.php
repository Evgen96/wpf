...
if( !function_exists('rehub_product_countdown') ) {
function rehub_product_countdown (){
	?>
		<!-- Countdown dashboard start --> 
		<?php wp_enqueue_script('lwtCountdown');?>
		<?php 
			$offset_year = vp_metabox('rehub_post.review_post.0.review_post_product_timer_year'); 
			$offset_month = vp_metabox('rehub_post.review_post.0.review_post_product_timer_month');
			$offset_day = vp_metabox('rehub_post.review_post.0.review_post_product_timer_day'); 
			$offset_hour = vp_metabox('rehub_post.review_post.0.review_post_product_timer_hour');
		?>
		<script type="text/javascript">
			jQuery(document).ready(function($) {
			  $('#countdown_dashboard').countDown({
				  targetDate: {
					  'day': 	<?php echo $offset_day ?>,
					  'month': 	<?php echo $offset_month ?>,
					  'year': 	<?php echo $offset_year ?>,
					  'hour': 	<?php echo $offset_hour ?>,
					  'min': 		0,
					  'sec': 		0
				  },
...
function rehub_get_offer(){
    ...
    <?php if (vp_metabox('rehub_post.review_post.0.review_post_schema_type') == 'review_post_review_product') : ?>
    	...
    	<?php if (vp_metabox('rehub_post.review_post.0.review_post_product_countdown') == '1') : ?><?php rehub_product_countdown () ?>  <?php endif ;?>
			...
			if ($linkpost) : ?>
				...
				<div class="rehub_feat_block table_view_block"><div class="block_with_coupon">
			        <?php if(!empty($offer_thumb) || (has_post_thumbnail())) : ?>
			   		...
			   		<?php endif ;?>
			   		<div class="desc_col">
			            <div class="offer_title"><?php echo $offer_title ;?></div>
			            <p><?php echo wp_kses_post($offer_desc); ?></p>
			        </div>
			        <?php if ( !empty($offer_price) || !empty($term_list[0])) :?>
				        <div class="price_col">
				        	<?php if(!empty($offer_price)) : ?><p> <span class="price_count"><ins><?php echo $offer_price ?></ins><?php if($offer_price_old !='') :?> <del><?php echo $offer_price_old ; ?></del><?php endif ;?></span></p><?php endif ;?>			                
				        	<div class="aff_tag">
		...
		<?php else :?>
			...
			<div class="rehub_feat_block table_view_block"><div class="block_with_coupon">
		        ...
		        <?php if(!empty($offer_price)) : ?>
		        	<div class="price_col">
		        		<p><span class="price_count"><ins><?php echo $offer_price ?></ins><?php if($offer_price_old !='') :?> <del><?php echo $offer_price_old ; ?></del><?php endif ;?></span></p>
		        	</div>
		        <?php endif ;?>		        
		        <div class="buttons_col">
		            <div class="priced_block clearfix">
    ...
    <?php if (vp_metabox('rehub_post.review_post.0.review_post_schema_type') == 'review_aff_product') : ?>
       	<div class="rehub_feat_block"><a name="aff-link-list"></a>
            ...
            <div class="aff_offer_desc"><?php if(!empty($aff_thumb) || (has_post_thumbnail())) : ?>
       		...
       		<?php endif ;?>
            <div class="offer_title"><?php echo $aff_title ;?></div>
			...
			if(function_exists('thirstyInit') && !empty($rehub_aff_post_ids)) :?>
				...
				<div class="rehub_feat_block table_view_block">
					...
					<div class="block_with_coupon">
						...
						<div class="desc_col">
							<div class="offer_title"><a href="<?php echo get_post_permalink($aff_post) ?>" target="_blank"><?php echo $aff_post->post_title; ?></a></div>
	...
	<?php global $woocommerce; if($woocommerce) :?>
		...
		<?php $products = new WP_Query( $args ); if ( $products->have_posts() ) : ?>                      
    		<?php while ( $products->have_posts() ) : $products->the_post(); global $product?>
	            ...
	            <?php $offer_url = get_post_permalink($review_woo_link) ?>
    			...
    			<div class="rehub_woo_review">
    				<?php if (!empty ($attributes) || !empty ($gallery_images)) :?>
						...
						<?php endif ;?>
						<div class="rehub_feat_block table_view_block">
			            <div class="rehub_woo_review_tabs" style="display:block">
				            <div class="offer_thumb">	
	...
	?>
	    ...
	    <?php elseif (vp_metabox('rehub_post.review_post.0.review_post_schema_type') == 'review_aff_product') :?> 
			...
			if(function_exists('thirstyInit') && !empty($rehub_aff_post_ids)) :?>
		        <div class="priced_block clearfix">
	                <?php $min_aff_price_count = get_post_meta(get_the_ID(), 'rehub_min_aff_price', true); if ($min_aff_price_count !='') : ?>
	                	<p><span class="price_count"><?php echo rehub_option('rehub_currency'); echo $min_aff_price_count; ?></span></p>
	                <?php endif ;?>		             
...
function coupon_get_code(){
	...
	if( !empty( $code ) ){
		$offer_coupon = get_post_meta( $code->ID, 'rehub_aff_coupon', true );
		$offer_link = get_the_permalink($code->ID);
