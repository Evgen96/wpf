...
function rehub_framework_register_scripts() {
	...
	wp_register_script('flexslider', get_template_directory_uri() . '/js/jquery.flexslider-min.js', 'jquery', '2.2.1', true);
...
function rehub_enqueue_scripts() {
	...
	wp_enqueue_style('default_font', 'http://fonts.googleapis.com/css?family=Open+Sans+Condensed:300,700&subset=latin,cyrillic');
...
function wpsm_thumb( $size = 'small' ){
	...
	if( rehub_option( 'aq_resize') == 1 ){
		if( rehub_option( 'aq_resize_crop') == '1') {$params = array( 'width' => $width, 'height' => $height);}
		else {$params = array( 'width' => $width, 'height' => $height, 'crop' => true);}
		$img = get_post_thumb(); 
		if(!empty($img)){ ?>
...
function parse_video_url($url,$return='embed',$width='',$height='',$rel=0){
  ...
  if(isset($yid)) {
    ...
    //return embed iframe
    if($return == 'embed'){
        return '<iframe width="'.($width?$width:765).'" height="'.($height?$height:430).'" src="//www.youtube.com/embed/'.$yid.'?rel='.$rel.'" frameborder="0" ebkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
...
function rehub_contactmethods( $contactmethods ) {
	...
	$contactmethods['twitter']   = __('Twitter Username', 'rehub_framework');
	$contactmethods['facebook']  = __('Facebook Username', 'rehub_framework');
	$contactmethods['google']    = __('Google Plus Username', 'rehub_framework');
	$contactmethods['tumblr']    = __('Tumblr Username', 'rehub_framework');
	$contactmethods['instagram'] = __('Instagram Username', 'rehub_framework');
	$contactmethods['vkontakte'] = __('Vk.com Username', 'rehub_framework');
	$contactmethods['youtube'] = __('Youtube Username', 'rehub_framework');
