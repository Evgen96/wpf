...
function wpsm_shortcode_button( $atts, $content = null ) {
	...
	$out = '<a href="'.$atts['link'].'" target="'.$atts['target'].'" style="border-radius:'.$atts['border_radius'].'" class="wpsm-button '.$atts['color'].' '.$atts['size'].'" rel="'.$atts['rel'].'"><span class="wpsm-button-inner" style="border-radius:'.$atts['border_radius'].'"><span class="fa fa-'.$atts['icon'].'"></span>' .do_shortcode($content). '</span></a>';
    return $out;
}
...
function recent_posts_function($atts, $content = null) {
   ...
   query_posts(array('orderby' => 'date', 'order' => 'DESC' , 'showposts' => $posts, 'category__in' => $catid));
   if (have_posts()) :
